...
class Review(BaseModel):
    Rating: Annotated[float, Field(description="Value between 0 and 5")]
    # rating: int = Field(ge=0, le=5, description="Value between 0 and 5") # another method to use it
    Pros: Annotated[List[str], Field(description="List of advantages")]
    Cons: Annotated[List[str], Field(description="List of disadvantages")]
    Summary: str
    Sentiment: Annotated[str, Field(description="positive, neutral, or negative")]
    PublisherName: Optional[str]
...
prompt = f"""
Extract structured data from the following text and return ONLY a valid JSON object with these fields:
- Rating (float, 0-5)
- Pros (list of strings)
- Cons (list of strings)
- Summary (string)
- Sentiment ("positive", "neutral", or "negative")
- PublisherName (string, only if mentioned)
